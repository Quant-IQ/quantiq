# Strategy Specification: EMA Crossover with Bollinger Bands Momentum Filter

## 1. Entry Signal
A buy signal is triggered when the fast 9-period Exponential Moving Average (EMA9) crosses above the slow 21-period Exponential Moving Average (EMA21), representing a bullish trend shift, AND the closing price is trading above the middle band of the 20-period Bollinger Bands (BB20, 2 std dev). This secondary filter ensures the crossover is backed by genuine price expansion and is not a low-volume false breakout.
...
2. The closing price falls back below the middle Bollinger Band (BB20), signaling a sudden loss of bullish momentum before the moving averages can mathematically cross.
...
Moving averages are lagging trend indicators that suffer heavily during sideways market phases. By combining the faster-reacting EMA crossover with the Bollinger Bands momentum filter, the system filters out choppy consolidation periods, executing trades only when price is expanding away from its mean with genuine directional velocity.

## 8. Known Weaknesses
The primary weaknesses include vulnerability to sudden, violent macroeconomic reversals where price gaps down instantly, bypassing the historical ATR trailing threshold before the exit trigger can fire. Additionally, during low-volatility contraction phases, the Bollinger Bands can narrow sharply, causing the middle-band filter to whipsaw with minor price fluctuations.
